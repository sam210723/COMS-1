    ...
    while True:
        ...
        if VCID == 63:
            ...
            if HEADER:
                ...
                # Allows TP_File to be skipped if first CP_PDU was missed (usually when demuxer is started)
                if SEQ == "FIRST":
                    seenFIRST = True
                    
                    # Remove previous TP_File
                    try:
                        os.remove(DIR_TEMP + '/TP.bin')
                    except OSError:
                        pass

                    # Create new TP_File
                    print("New TP_File")
                    tempTPFile = open(DIR_TEMP + '/TP.bin', 'wb+')

                #TODO: Check CP_PDU counter continuity

                # Handle completed CP_PDU
                if (POINTER == 0 and SEQ == "FIRST") or (POINTER != 0 and currentCPPDU != None):
                    ...
                    if not checkCRC(currentCPPDU, currentCRC, crcLUT):
                        ...
                        if seenFIRST and not tempTPFile.closed:
                            # Append CP_PDU to current TP_File
                            tempTPFile.write(currentCPPDU)
                        
                        if SEQ == "LAST" and seenFIRST:
                            # Close TP_File
                            tempTPFile.close()
                            print("Closed TP_File")

                            shutil.copyfile(DIR_TEMP + "/TP.bin", DIR_TEMP + "/LAST-TP.bin")
